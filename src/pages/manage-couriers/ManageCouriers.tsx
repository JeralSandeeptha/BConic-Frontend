...
const ManageCouriers = (props: ManageCouriersPageProps) => {
  ...
  useEffect(() => {
    const results = courieres.filter(
      (courier) =>
    ...
    <div className="test manage-couriers">
      ...
      <div className="test search-section">
        <div className="test searchbar">
          ...
          />
          <img
            src="https://res.cloudinary.com/dv9ax00l4/image/upload/v1722774870/icons8-search-480_msptia.png"
            alt="search-logo"
            className="test search-logo"
          />
        </div>
        <div className="test search-option-section">
          ...
          >
            <option value={COURIER_STATUS.ORDER_PLACED} className="test option">
              {COURIER_STATUS.ORDER_PLACED}
            </option>
            <option value={COURIER_STATUS.PICKED_UP} className="test option">
              {COURIER_STATUS.PICKED_UP}
            </option>
            <option value={COURIER_STATUS.IN_TRANSIT} className="test option">
              {COURIER_STATUS.IN_TRANSIT}
            </option>
            <option
              value={COURIER_STATUS.OUT_FOR_DELIVERY}
              className="test option"
            >
              {COURIER_STATUS.OUT_FOR_DELIVERY}
            </option>
            <option
              value={COURIER_STATUS.ATTEMPTED_DELIVERY}
              className="test option"
            >
              {COURIER_STATUS.ATTEMPTED_DELIVERY}
            </option>
            <option
              value={COURIER_STATUS.RETURNED_TO_SENDER}
              className="test option"
            >
              {COURIER_STATUS.RETURNED_TO_SENDER}
            </option>
            <option value={COURIER_STATUS.ON_HOLD} className="test option">
              {COURIER_STATUS.ON_HOLD}
            </option>
            <option value={COURIER_STATUS.CANCELLED} className="test option">
              {COURIER_STATUS.CANCELLED}
            </option>
            <option value={COURIER_STATUS.DELAYED} className="test option">
              {COURIER_STATUS.DELAYED}
            </option>
            <option value={COURIER_STATUS.PROCESSING} className="test option">
              {COURIER_STATUS.PROCESSING}
            </option>
            <option value={COURIER_STATUS.LOST} className="test option">
              {COURIER_STATUS.LOST}
            </option>
          </select>
        </div>
        <Link
          to="/dashboard/add-courier"
          className="test add-manage-couriers-btn-link"
        >
          <button className="test add-manage-couriers-btn">Add Courier</button>
        </Link>
      </div>

      <div className="test search-results">
        <h6 className="test search-results-text">
          Search Results :{" "}
          <span className="test result-count">{filteredCouriers.length}</span>
        </h6>
      </div>
      ...
      <div className="test manage-couriers-couriers-table">
        ...
        <div className="test table-body">
          {filteredCouriers.length === 0 ? (
            <NoData message="No couriers data available" />
          ) : (
